...
class String
  ...
  def current_color;  self.bg_cyan.gray        end
...
class JPSAlgorithm
  ...
  def search_done?
    return (@route.found || @open_list.empty?)
  end
  ...
  def search_step
    @use_jps ? search_jps_step : search_astar_step
  end
  ...
  def get_cell_string(path, n)
    if n == nil
      return "_" * (@render_cell_width - 1)
    else
      colorizer = case
                  when n.id == @route.start_id;   :start_color
                  when n.id == @route.goal_id;    :goal_color
                  when @obstacles.include?(n.id); :obstacle_color
                  when path != nil && path.find{|node| node.id == n.id} != nil; :path_color
                  when search_done?()==false && @open_list.empty? == false && (@open_list.last.id == n.id); :current_color
      ...
      return %Q{#{symbol}#{" " * (@render_cell_width - symbol.length)}}.send(colorizer)
    end
  end
  private :get_cell_string
...
#  jps.search
  ...
  until jps.search_done?
    jps.search_step
    jps.render #while true
  end
